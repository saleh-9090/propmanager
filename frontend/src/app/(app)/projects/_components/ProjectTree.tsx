...
export default function ProjectTree() {
  ...
  if (loading) return <div className="p-4 text-sm text-text-secondary">جارٍ التحميل...</div>

  return (
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-border flex items-center justify-between">
        <h2 className="font-semibold text-sm text-text-primary">المشاريع</h2>
        <button
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
        {projects.length === 0 && (
          <p className="text-sm text-text-muted p-2 text-center mt-8">
            لا توجد مشاريع
          </p>
        )}
        {projects.map(project => (
          <div key={project.id}>
            <div className="flex items-center gap-1 px-2 py-1.5 rounded-lg hover:bg-bg-elevated group">
              <button
                ...
                className="text-text-muted w-4 text-xs shrink-0"
              >
              ...
              <div className="hidden group-hover:flex items-center gap-1">
                <button
                  ...
                  className="text-xs text-brand-primary px-1"
                  title="إضافة مبنى"
                >+ مبنى</button>
                <button
                ...
                <button
                  ...
                  className="text-xs text-danger/70 hover:text-danger px-1"
                >×</button>
              </div>
            </div>
            ...
            {expanded.has(project.id) && (
              <div className="mr-5 space-y-0.5">
                {project.buildings.length === 0 && (
                  <p className="text-xs text-text-muted px-4 py-1">لا توجد مبانٍ</p>
                )}
                {project.buildings.map(b => (
                  ...
                  >
                    <span className="w-4 text-center text-text-muted text-xs shrink-0">■</span>
